package titles.binary_tree.bst.red_black_tree;/*
Design Decisions:
-----------------
...
//---------------------------------------------- verify code ---------------------------------------------------------------------------------
	...
	private void inorderTraversal(RedBlackNode<T> root) {
		if (root != nil) {
	...
	private void printHelper(RedBlackNode<T> root, String indent, boolean last) {
		if (root != nil) {
			...
			String sColor = root.color == RedBlackNode.RED ? "RED" : "BLACK";
	...
	void finalPrint(){
		...
		System.out.println();
		printTree();
		// sleep 100 millis for getting unrepeated numbers as possible
		try {
			Thread.sleep( 100);
		} catch (InterruptedException e) {
			Thread.currentThread().interrupt(); // recover from interruption
		}
	}

	// check if it's red-black tree
	public boolean isRedBlackTree(RedBlackNode<T> root) {
		if (root == nil) return true;

		// the root must be black
		if (root.color==RedBlackNode.RED) return false;

		// check other features
		return checkRedBlackProperties(root, 0, -1);
	}

	private boolean checkRedBlackProperties(RedBlackNode<T> node, int blackCount, int pathBlackCount) {
		if (node == nil) {
			// reach the leaf node（nil node）
			if (pathBlackCount == -1) {
				pathBlackCount = blackCount;
			} else if (blackCount != pathBlackCount) {
				// the number of black nodes on the path from the root node to the leaf node must be the same
				return false;
			}
			return true;
		}
		...
		if ((node.left != nil) && (node.key.compareTo(node.left.key) < 0)) {
			return false;
		}
		if ((node.right != nil) && (node.key.compareTo(node.right.key) > 0)) {
			return false;
		}

		// sons of red nodes must be black
		if (node.color==RedBlackNode.RED) {
			if ((node.left != nil && node.left.color==RedBlackNode.RED) || (node.right != nil && node.right.color==RedBlackNode.RED)) {
				return false;
			}
		} else {
			blackCount++;
		...
		// recursion to the left and right
	...
	public static void main(String[] args) {
		...
		for (int j=0; j<400; ++j) {
			...
			for (int i = 0; i < 400; i++) { // default to be 10000, 150 to test
				int value = rand.nextInt(10000) + 1; // Random value between 1 and 10000
				randomNums[i]=value;
				tree.insert(value);
				tree.finalPrint();
				if (!tree.isRedBlackTree(tree.root)) {
					System.out.println("Tree is un-red-black tree after inserting " + value);
					break;
				}
			}
			shuffle(randomNums); // shuffle the nums
			for (int i = 0; i < 400; i++) {
				int value = randomNums[i];
				RedBlackNode<Integer> node = tree.search(value);
				if(node!=null)tree.remove(node);
				tree.finalPrint();
				if (!tree.isRedBlackTree(tree.root)) {
					System.out.println("Tree is un-red-black tree after deleting " + value);
					break;
				}
			}
		}
